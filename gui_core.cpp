...
DWORD __stdcall GuiCore::KeybindThread()
{
	try
	{
		do
			if (GetAsyncKeyState(VK_INSERT) & 0x1 || GetAsyncKeyState(VK_RCONTROL) & 0x1 || GetAsyncKeyState(VK_F11) & 0x1)
				Menu::isOpen = !Menu::isOpen;
			else
				std::this_thread::sleep_for(std::chrono::milliseconds(1));
		while (true);
	}
	catch (...) { MessageBoxA(NULL, "keybinds crashed", "redENGINE", MB_OK); }
}